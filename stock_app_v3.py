...
# (其他函數如 get_comprehensive_data, fetch_price_data 等請保留原本您確認能運作的定義)
...
if st.session_state.selected_stock:
    # 詳情頁 (保留您的五指標邏輯)
    if st.button("⬅ 返回"): st.session_state.selected_stock = None; st.rerun()
    # [這裡填入您原本完整的詳情頁繪圖程式碼]
else:
    # 大廳頁
    st.title("📈 專業台股控盤系統")
    ...
    pages = (len(filtered) + 8) // 9
    current = filtered[st.session_state.page * 9 : (st.session_state.page + 1) * 9]
    ...
    for i in range(0, len(current), 3):
        ...
        for j, col in enumerate(cols):
            if i+j < len(current):
                s = current[i+j]
                # 這裡使用 staticPlot=True 確保不會因為圖表導致頁面崩潰
                # [在 col 裡面放入簡易價格與圖表]
                if col.button(f"查看 {s['name']}", key=s['id']):
                    st.session_state.selected_stock = s['id']
                    st.rerun()
                    
    if st.button("下一頁") and st.session_state.page < pages-1: st.session_state.page += 1; st.rerun()
