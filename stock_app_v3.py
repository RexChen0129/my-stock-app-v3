...
if st.session_state.selected_stock:
    ...
    st.title(f"股票分析: {st.session_state.selected_stock}")
    # 此處執行您原有的五指標繪圖邏輯 (K線, 成交量, 法人, KD, MACD)
else:
    # --- 大廳頁 ---
    st.title("📈 台股自選股大廳")
    search = st.text_input("🔍 搜尋名稱或代號", value=st.session_state.search_query)
    ...
    # 篩選邏輯
    filtered = [s for s in WATCHLIST if search in s['id'] or search in s['name']]
    ...
    for i, stock in enumerate(filtered[start : start + 9]):
        with cols[i % 3]:
            ...
            # 渲染迷你 K 線 (這裡 config 設為 staticPlot 避免崩潰)
            # st.plotly_chart(get_mini_chart(stock['id']), config={'staticPlot': True}) 
            if st.button(f"進入分析", key=stock['id']):
